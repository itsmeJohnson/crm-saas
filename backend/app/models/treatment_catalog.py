...
class TreatmentCatalogItem(BaseModel):
    """A tenant's master list of treatments/procedures with prices — the
    'price master' used to populate treatment plans and invoices."""
    __tablename__ = "treatment_catalog_items"

    organization_id: Mapped[uuid.UUID] = mapped_column(ForeignKey("organizations.id"), nullable=False, index=True)
    name: Mapped[str] = mapped_column(String(200), nullable=False)
    category: Mapped[str | None] = mapped_column(String(100), nullable=True, index=True)
    code: Mapped[str | None] = mapped_column(String(40), nullable=True)  # procedure / SKU code
