...
from app.dependencies.feature_guard import require_feature as _require_feature

def _rbac(resource: str):
    return [_Depends(_enforce(resource))]

def _feat(code: str):
    """Plan-tier gate: 403 unless the tenant's plan includes `code` (SuperAdmin bypasses)."""
    return [_Depends(_require_feature(code))]
...
app.include_router(org_analytics_router,   prefix=f"{settings.API_V1_STR}/org-analytics",   tags=["org-analytics"], dependencies=_rbac("analytics") + _feat("MANAGER_DASHBOARD"))
app.include_router(automation_analytics_router, prefix=f"{settings.API_V1_STR}/automation-analytics", tags=["automation-analytics"], dependencies=_rbac("analytics") + _feat("ADVANCED_ANALYTICS"))
app.include_router(executive_dashboard_router, prefix=f"{settings.API_V1_STR}/executive-dashboard", tags=["executive-dashboard"], dependencies=_rbac("analytics") + _feat("MANAGER_DASHBOARD"))
app.include_router(report_builder_router,  prefix=f"{settings.API_V1_STR}/report-builder",   tags=["report-builder"], dependencies=_rbac("analytics") + _feat("CUSTOM_REPORTS"))
app.include_router(sales_analytics_router, prefix=f"{settings.API_V1_STR}/sales-analytics",   tags=["sales-analytics"], dependencies=_rbac("analytics") + _feat("ADVANCED_ANALYTICS"))
app.include_router(employee_analytics_router, prefix=f"{settings.API_V1_STR}/employee-analytics", tags=["employee-analytics"], dependencies=_rbac("analytics") + _feat("TEAM_MONITORING"))
app.include_router(financial_analytics_router, prefix=f"{settings.API_V1_STR}/financial-analytics", tags=["financial-analytics"], dependencies=_rbac("analytics") + _feat("ADVANCED_ANALYTICS"))
app.include_router(forecasting_router,     prefix=f"{settings.API_V1_STR}/forecasting",      tags=["forecasting"], dependencies=_rbac("analytics") + _feat("ADVANCED_ANALYTICS"))
app.include_router(kpi_router,              prefix=f"{settings.API_V1_STR}/kpi",              tags=["kpi"], dependencies=_rbac("analytics") + _feat("KPI_DASHBOARD"))
app.include_router(okr_router,              prefix=f"{settings.API_V1_STR}/okr",              tags=["okr"], dependencies=_rbac("targets") + _feat("TARGET_MANAGEMENT"))
app.include_router(visualizations_router,   prefix=f"{settings.API_V1_STR}/visualizations",   tags=["visualizations"], dependencies=_rbac("analytics") + _feat("ADVANCED_ANALYTICS"))
app.include_router(scheduled_reports_router, prefix=f"{settings.API_V1_STR}/scheduled-reports", tags=["scheduled-reports"], dependencies=_rbac("analytics") + _feat("CUSTOM_REPORTS"))
...
app.include_router(bi_export_router,        prefix=f"{settings.API_V1_STR}/bi",               tags=["bi"], dependencies=_rbac("analytics") + _feat("ADVANCED_ANALYTICS"))
app.include_router(historical_analytics_router, prefix=f"{settings.API_V1_STR}/historical-analytics", tags=["historical-analytics"], dependencies=_rbac("analytics") + _feat("ADVANCED_ANALYTICS"))
app.include_router(compliance_router,       prefix=f"{settings.API_V1_STR}/compliance",       tags=["compliance"], dependencies=_rbac("analytics") + _feat("ADVANCED_ANALYTICS"))
app.include_router(predictive_router,       prefix=f"{settings.API_V1_STR}/predictive",       tags=["predictive"], dependencies=_rbac("analytics") + _feat("ADVANCED_ANALYTICS"))
app.include_router(ai_platform_router,      prefix=f"{settings.API_V1_STR}/ai",               tags=["ai"], dependencies=_rbac("ai") + _feat("AI_CALL_SUMMARY"))
app.include_router(copilot_router,          prefix=f"{settings.API_V1_STR}/copilot",          tags=["copilot"], dependencies=_rbac("ai") + _feat("AI_CALL_SUMMARY"))
app.include_router(lead_intelligence_router, prefix=f"{settings.API_V1_STR}/lead-intelligence", tags=["lead-intelligence"], dependencies=_rbac("leads") + _feat("AI_CALL_SUMMARY"))
...
app.include_router(comm_intelligence_router, prefix=f"{settings.API_V1_STR}/comm-intelligence", tags=["comm-intelligence"], dependencies=_feat("AI_CALL_SUMMARY"))
app.include_router(sales_intelligence_router, prefix=f"{settings.API_V1_STR}/sales-intelligence", tags=["sales-intelligence"], dependencies=_rbac("analytics") + _feat("AI_CALL_SUMMARY"))
app.include_router(knowledge_router,        prefix=f"{settings.API_V1_STR}/knowledge",        tags=["knowledge"], dependencies=_rbac("ai") + _feat("AI_CALL_SUMMARY"))
app.include_router(document_intelligence_router, prefix=f"{settings.API_V1_STR}/document-intelligence", tags=["document-intelligence"], dependencies=_rbac("ai") + _feat("AI_CALL_SUMMARY"))
app.include_router(workflow_assistant_router,   prefix=f"{settings.API_V1_STR}/workflow-assistant", tags=["workflow-assistant"], dependencies=_rbac("workflows") + _feat("AI_FOLLOW_UP"))
app.include_router(prediction_engine_router,    prefix=f"{settings.API_V1_STR}/prediction-engine", tags=["prediction-engine"], dependencies=_rbac("analytics") + _feat("AI_CALL_SUMMARY"))
app.include_router(recommendations_router,      prefix=f"{settings.API_V1_STR}/recommendations", tags=["recommendations"], dependencies=_rbac("ai") + _feat("AI_FOLLOW_UP"))
app.include_router(prompt_studio_router,        prefix=f"{settings.API_V1_STR}/prompt-studio",   tags=["prompt-studio"], dependencies=_rbac("ai") + _feat("AI_CALL_SUMMARY"))
app.include_router(ai_governance_router,         prefix=f"{settings.API_V1_STR}/ai-governance",   tags=["ai-governance"], dependencies=_rbac("ai") + _feat("AI_CALL_SUMMARY"))
app.include_router(ai_analytics_router,          prefix=f"{settings.API_V1_STR}/ai-analytics",    tags=["ai-analytics"], dependencies=_rbac("ai") + _feat("AI_CALL_SUMMARY"))
app.include_router(ai_public_api_router,         prefix=f"{settings.API_V1_STR}/ai-api",          tags=["ai-api"])
app.include_router(ai_developer_router,          prefix=f"{settings.API_V1_STR}/ai-developer",    tags=["ai-developer"], dependencies=_rbac("ai") + _feat("API_ACCESS"))
...
app.include_router(integrations_router,          prefix=f"{settings.API_V1_STR}/integrations",    tags=["integrations"], dependencies=_rbac("integrations") + _feat("API_ACCESS"))
app.include_router(workflows_router,       prefix=f"{settings.API_V1_STR}/workflows",       tags=["workflows"], dependencies=_rbac("workflows") + _feat("ADVANCED_ANALYTICS"))
app.include_router(rules_router,           prefix=f"{settings.API_V1_STR}/rules",           tags=["rules"], dependencies=_rbac("rules") + _feat("ADVANCED_ANALYTICS"))
app.include_router(automation_router,      prefix=f"{settings.API_V1_STR}/automation",      tags=["automation"], dependencies=_rbac("automation") + _feat("ADVANCED_ANALYTICS"))
app.include_router(events_router,          prefix=f"{settings.API_V1_STR}/events",          tags=["events"], dependencies=_rbac("events") + _feat("ADVANCED_ANALYTICS"))
app.include_router(queue_router,           prefix=f"{settings.API_V1_STR}/queue",           tags=["queue"], dependencies=_rbac("queue") + _feat("ADVANCED_ANALYTICS"))
app.include_router(scheduler_router,       prefix=f"{settings.API_V1_STR}/scheduler",       tags=["scheduler"], dependencies=_rbac("scheduler") + _feat("ADVANCED_ANALYTICS"))
app.include_router(notification_automation_router, prefix=f"{settings.API_V1_STR}/notification-automation", tags=["notification-automation"], dependencies=_rbac("notifications") + _feat("ADVANCED_ANALYTICS"))
app.include_router(sla_router,             prefix=f"{settings.API_V1_STR}/sla",             tags=["sla"], dependencies=_rbac("sla") + _feat("ADVANCED_ANALYTICS"))
app.include_router(escalation_router,      prefix=f"{settings.API_V1_STR}/escalation",      tags=["escalation"], dependencies=_rbac("escalation") + _feat("ADVANCED_ANALYTICS"))
