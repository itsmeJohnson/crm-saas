...
import { PhoneCall, PhoneMissed, Timer, Percent, Loader2 } from 'lucide-react';
...
const DayTrend: React.FC<{ buckets: { label: string; count: number }[] }> = ({ buckets }) => {
  ...
  return (
    <div className="glass-panel border border-slate-800/85 rounded-2xl p-5">
      <h3 className="text-sm font-semibold text-slate-200 mb-4">Calls per Day</h3>
        ...
        <div className="flex items-end gap-1.5 h-28">
          {recent.map((b) => (
            <div key={b.label} className="flex-1 flex flex-col items-center gap-1" title={`${b.label}: ${b.count}`}>
              ...
              <span className="text-[8px] text-slate-600 rotate-0">{b.label.slice(5)}</span>
            </div>
          ))}
        </div>
      )}
...
export const CallingReportsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const params = days
      ? { date_from: new Date(Date.now() - days * 24 * 3600 * 1000).toISOString() }
      : {};
    callingApi.reports(params).then(setReport).catch(() => {}).finally(() => setIsLoading(false));
  }, [range]);

  const fmtAvg = (s: number) => (s >= 60 ? `${Math.floor(s / 60)}m ${s % 60}s` : `${s}s`);

  const stats = report ? [
    { label: 'Total Calls', value: String(report.total), icon: PhoneCall, color: 'text-brand-400' },
    { label: 'Connect Rate', value: `${report.connect_rate}% (${report.connected}/${report.dispositioned})`, icon: Percent, color: 'text-emerald-400' },
    { label: 'Avg Duration', value: fmtAvg(report.avg_duration), icon: Timer, color: 'text-amber-400' },
    { label: 'Missed', value: String(report.missed), icon: PhoneMissed, color: 'text-red-400' },
  ] : [];
  ...
  return (
    <div className="space-y-6">
      <div className="border-b border-slate-800/60 pb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-slate-100 to-slate-400 bg-clip-text text-transparent">Call Reports</h1>
          <p className="text-sm text-slate-400 mt-1">Volume, connect rate, duration, and agent performance.</p>
        </div>
      ...
      {isLoading ? <div className="py-24 text-center text-slate-400"><Loader2 className="w-6 h-6 animate-spin inline" /></div> : report ? (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Bar title="By Disposition" buckets={report.by_disposition} />
            <Bar title="By Agent" buckets={report.by_agent} />
          </div>
        </>
      ) : null}
    </div>
