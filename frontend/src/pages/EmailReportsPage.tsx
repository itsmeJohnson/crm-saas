...
import { Mail, Eye, MousePointerClick, XCircle, Loader2 } from 'lucide-react';
...
const DayTrend: React.FC<{ buckets: { label: string; count: number }[] }> = ({ buckets }) => {
  ...
  return (
    <div className="glass-panel border border-slate-800/85 rounded-2xl p-5">
      <h3 className="text-sm font-semibold text-slate-200 mb-4">Emails per Day</h3>
...
export const EmailReportsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    emailApi.reports(params).then(setReport).catch(() => {}).finally(() => setIsLoading(false));
  }, [range]);

  const stats = report ? [
    { label: 'Sent', value: String(report.sent), icon: Mail, color: 'text-brand-400' },
    { label: 'Open Rate', value: `${report.open_rate}% (${report.opened}/${report.sent})`, icon: Eye, color: 'text-emerald-400' },
    { label: 'Click Rate', value: `${report.click_rate}% (${report.clicked}/${report.sent})`, icon: MousePointerClick, color: 'text-sky-400' },
  ...
  return (
    <div className="space-y-6">
      <div className="border-b border-slate-800/60 pb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight bg-gradient-to-r from-slate-100 to-slate-400 bg-clip-text text-transparent">Email Reports</h1>
          <p className="text-sm text-slate-400 mt-1">Open + click engagement, volume, and delivery.</p>
        </div>
      ...
      {isLoading ? <div className="py-24 text-center text-slate-400"><Loader2 className="w-6 h-6 animate-spin inline" /></div> : report ? (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {stats.map((st) => (
              <div key={st.label} className="glass-panel border border-slate-800/85 rounded-2xl p-5">
                <div className="flex items-center gap-2 mb-2"><st.icon className={`w-4 h-4 ${st.color}`} /><span className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">{st.label}</span></div>
                <p className="text-xl font-bold text-slate-100">{st.value}</p>
              </div>
            ))}
          </div>
